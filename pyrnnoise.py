...
class RNNoise:
    ...
    def denoise_frame(
        self, frame: np.ndarray, partial: bool = False
        ...
        if self.sample_rate != SAMPLE_RATE:
            ...
            denoised_frame = np.concatenate(
                [frame for frame, _ in self.out_graph.pull(partial)], axis=1
            )
    ...
    def denoise_wav(self, in_path, out_path):
        ...
        for idx, (frame, _) in tqdm(
            enumerate(reader), desc="Denoising", total=reader.num_frames, unit="frames"
        ):
